...
const $productNavBar = $('.product-navigation-bar');

if ($productNavBar.length) {
    import(/* webpackChunkName: "flickity" */ 'flickity').then(flickity => {
        $productNavBar.find('.slide-wrapper').on('click','.js-slide', function(e){
            var index = $(e.currentTarget).index();
            $('.productImgSlider').flickity('select',index);
        })
    });
}
